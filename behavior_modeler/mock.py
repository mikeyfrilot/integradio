...
class MockFlowGenerator:
    ...
    def generate_session(
        ...
        # Generate deterministic session ID from our seeded RNG
        session_id = session_id or f"{self.rng.randint(0, 0xffffffff):08x}-{self.rng.randint(0, 0xffff):04x}-{self.rng.randint(0, 0xffff):04x}-{self.rng.randint(0, 0xffff):04x}-{self.rng.randint(0, 0xffffffffffff):012x}"
